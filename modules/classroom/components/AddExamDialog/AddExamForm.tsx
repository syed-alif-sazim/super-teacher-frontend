...
import { TAddExamForm } from './AddExamForm.types';
import { addExamFormSchema } from './AddExamForm.schema';
    ...
    return (
        <Form {...form}>
          ...
          <div className="flex flex-col space-y-4 md:space-y-0 ">
            ...
            render={({ field }) => (
                <FormItem className="flex-1">
                <FormLabel className='text-custom-green text-md font-bold'>Schedule Date</FormLabel>
                <FormControl>
                <Input
                  type="datetime-local"
                  {...field}
                  value={
                    field.value
                      ? new Date(new Date(field.value).getTime() + SIX_HOURS_IN_MS)
                          .toISOString()
                          .slice(0, 16)
                      : ""
                  }
                ...
                <FormMessage className={cn("text-red-500 mt-1 h-2 sm:h-8",!errors["scheduleDate"]? " opacity-0" :"" )} >
